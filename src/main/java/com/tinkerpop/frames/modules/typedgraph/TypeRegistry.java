...
/**
 * A TypeRegistry for a {@link FramedGraph}. With a {@link TypeRegistry} the {@link FramedGraph} is able to store type-information on edges, and use
 * stored type-information to construct vertices/edges based on type-information stored in the graph (runtime).
 * 
 * @see TypeField
 * @see TypeValue
 * @see FramedGraph
...
public class TypeRegistry
{
    ...
    /**
     * @return The interface that has the {@link TypeField} annotation for this class. (Either the class itself, or a base class if the class was
     *         registered).
     */
    ...
    public TypeRegistry add(Class<?> type)
    {
        ...
        if (!typeFields.containsKey(type))
        {
            ...
            Validate.assertArgument(typeHoldingTypeField != null, "The type and its supertypes don't have a @TypeField annotation: %s",
                        type.getName());
    ...
    private Class<?> findTypeHoldingTypeField(Class<?> type)
    {
        ...
        for (Class<?> parentType : type.getInterfaces())
        {
            ...
            Validate.assertArgument(
                        parentTypeHoldingTypeField == null || typeHoldingTypeField == null || parentTypeHoldingTypeField == typeHoldingTypeField,
